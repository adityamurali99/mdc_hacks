      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
        ...
        {imageUrls?.length > 0 && (
          <div style={{
            ...
            {/* Matching / mismatching features */}
            {(audit_log?.visual_verification?.matching_features?.length > 0 || audit_log?.visual_verification?.mismatching_features?.length > 0) && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderTop: "1px solid #1c1c2e" }}>
                {/* Matching */}
                <div style={{ padding: "12px", borderRight: "1px solid #1c1c2e" }}>
                  <p style={{ fontFamily: F.mono, fontSize: "10px", color: T.safe, letterSpacing: "0.1em", marginBottom: "8px", marginTop: 0 }}>✓ MATCHING</p>
                  {audit_log.visual_verification.matching_features?.length > 0 ? (
                    audit_log.visual_verification.matching_features.map((f, i) => (
                      <div key={i} style={{ display: "flex", gap: "6px", marginBottom: "4px" }}>
                        <span style={{ color: T.safe, fontSize: "10px", marginTop: "2px" }}>▸</span>
                        <span style={{ fontFamily: F.body, fontSize: "12px", color: T.mid, lineHeight: "1.4" }}>{f}</span>
                      </div>
                    ))
                  ) : (
                    <span style={{ fontFamily: F.body, fontSize: "12px", color: T.dim, fontStyle: "italic" }}>No matching features found</span>
                  )}
                </div>
                {/* Mismatching */}
                <div style={{ padding: "12px" }}>
                  <p style={{ fontFamily: F.mono, fontSize: "10px", color: T.danger, letterSpacing: "0.1em", marginBottom: "8px", marginTop: 0 }}>✗ MISMATCHING</p>
                  {audit_log.visual_verification.mismatching_features?.length > 0 ? (
                    audit_log.visual_verification.mismatching_features.map((f, i) => (
                      <div key={i} style={{ display: "flex", gap: "6px", marginBottom: "4px" }}>
                        <span style={{ color: T.danger, fontSize: "10px", marginTop: "2px" }}>▸</span>
                        <span style={{ fontFamily: F.body, fontSize: "12px", color: T.mid, lineHeight: "1.4" }}>{f}</span>
                      </div>
                    ))
                  ) : (
                    <span style={{ fontFamily: F.body, fontSize: "12px", color: T.dim, fontStyle: "italic" }}>No mismatches detected</span>
                  )}
                </div>
              </div>
            )}

            {/* Additional listing photos strip */}
            {imageUrls?.length > 1 && (
              <div style={{ padding: "10px 12px", borderTop: "1px solid #1c1c2e", display: "flex", gap: "6px", overflowX: "auto" }}>
                <span style={{ fontFamily: F.mono, fontSize: "10px", color: T.dim, alignSelf: "center", flexShrink: 0, marginRight: "4px" }}>ALL PHOTOS</span>
                {imageUrls.map((url, i) => (
                  <img key={i} src={url} alt={`Photo ${i + 1}`}
                    style={{ height: "52px", width: "70px", objectFit: "cover", borderRadius: "6px", border: `1px solid ${i === 0 ? "#e8ff4740" : "#1c1c2e"}`, flexShrink: 0 }} />
                ))}
              </div>
            )}
          </div>
